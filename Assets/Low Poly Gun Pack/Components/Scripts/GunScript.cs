...
public class GunScript : MonoBehaviour	{
	...
	void Start ()
	{
		...
		if (!WeaponType.rpg && !WeaponType.handgunSilencer && !WeaponType.smgSilencer && !WeaponType.sniperSilencer 
		    && !WeaponType.assaultRifleSilencer && !WeaponType.assaultRifleSilencer2 && !WeaponType.grenadeLauncher && !WeaponType.handGrenade && !WeaponType.flamethrower) {
	...
	IEnumerator Muzzleflash ()
	{
		...
		if (!WeaponType.rpg && !WeaponType.grenadeLauncher) {
			...
			if (Physics.Raycast (bulletSpawnPoint.transform.position, 
				...
				if (hit.transform.tag == bodyImpactStaticTag) {
					//Spawn bullet impact on surface
					Instantiate (bodyImpactStaticPrefab, hit.point, 
						Quaternion.FromToRotation (Vector3.forward, hit.normal)); 
				}

				if (hit.transform.tag == bodyImpactTag) {
					//Spawn bullet impact on surface
					Instantiate (bodyImpactPrefab, hit.point, 
						Quaternion.FromToRotation (Vector3.forward, hit.normal));
	...
	void Update ()
	{
		...
		//If flamethrower is true
		if (WeaponType.flamethrower == true) {
			...
			if (Input.GetMouseButton (0) && !outOfAmmo && !isReloading) {
				...
				Components.flameParticles.GetComponent<ParticleSystem>().Emit(1);
